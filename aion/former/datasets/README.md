...
**`aion.former.datasets`** provides **tokenization** and a **sliding-window text dataset** for **next-token prediction** inside `aion.former`. For generic file I/O (streaming lines, atomic writes, checksums), use **`aion.io`** or the standard library.
